...
def test_quarter_and_year_have_no_calendar_window():
    for key in ("quarter", "year"):
        with pytest.raises(ValueError):
            periods.latest_window(key)
...
def test_a_fresh_install_owes_all_three(db):
    assert summaries.due_periods(db, at(2026, 9, 8, 6)) == ["day", "week", "month"]


def test_a_settled_morning_owes_nothing(db, player):
    now = at(2026, 9, 8, 6)
    written(db, player, now, ("day", "week", "month"))
    assert summaries.due_periods(db, at(2026, 9, 8, 8)) == []


def test_monday_adds_the_week_and_the_first_adds_the_month(db, player):
    written(db, player, at(2026, 9, 13, 6), ("day", "week", "month"))
    assert summaries.due_periods(db, at(2026, 9, 14, 6)) == ["day", "week"]

    written(db, player, at(2026, 9, 30, 6), ("day", "week", "month"))
    assert summaries.due_periods(db, at(2026, 10, 1, 6)) == ["day", "month"]
